...
/**
 * Base44 sometimes returns null/undefined or an object wrapper like:
 * { data: [...] } or { results: [...] } or { items: [...] }
 * This guarantees an array so .map/.filter never crash.
 */
const norm = (v) => {
...
export default function OwnerDashboard() {
  ...
  // Staff restaurants (IMPORTANT: guard map)
  const staffRestaurantsQ = useQuery({
    ...
    queryFn: async () => {
      const assigns = norm(staffAssignments);
  ...
  // All restaurants (de-dupe + always array)
  const allRestaurants = useMemo(() => {
    const merged = [...norm(ownedRestaurants), ...norm(staffRestaurants)].filter(Boolean);
    const seen = new Set();
  ...
  // Keep selected restaurant valid
  useEffect(() => {
    ...
    if (!allRestaurants.some(r => r.id === selectedRestaurant.id)) {
      setSelectedRestaurant(allRestaurants[0]);
    }
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          {allRestaurants.length > 1 && (
            <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
              {norm(allRestaurants).map((restaurant) => (
                <button
                  key={restaurant.id}
